...
ここでいう選択とは `handleAdd` / `handleFileChange` の 1 回の呼び出し、つまり選んだファイルを
フォームへ反映し終えるまでの処理を指す（`handleDelete` / `handleMove` は await の前に書き終える
ので含まない）。反映の前に await を挟む（`processFile` があればその分長い）ので、その窓で競合が起きる。

判定は「現行の選択」1 本で行う。**競合単位ごとに現行を 1 つだけ持ち、その値の参照が「現行は自分だ」
という印になる。** 選択は現行として登録されてから走り、解決したら自分がまだ現行かを参照比較で
確かめてから書く（転送側が `isCurrent` で持っているのと同じ規則）。登録が await より前にあるので、
先着は解決時点で既に現行ではなく、`ImageUtils.replaceExisting` の commit を undo せずに済む。
競合単位を生成された項目の参照にしないのは、`replaceExisting` が選び直しのたびに新しい tempId を
採番するため。

この 1 つの仕組みで 3 つの結末が決まる。**後着が勝つ**（交代で先着が現行を降りる）、
**削除が勝つ**（`handleDelete` が降ろす）、**unmount を跨いだ選択は捨てる**（cleanup が降ろす）。
最後の 1 つが要るのは、アダプタの `images` が unmount 時点で凍結される一方 `setImages` は生きた
フォームへ書くため、その commit が再 mount 後に追加された項目を巻き戻すから。

`uploads.wait` は同じ現行の集合を待つ。待機はそのスナップショットに対して行うので、現行を降ろす
ときは待ち側へ明示的に伝える（`CurrentSelection.displace`）。伝えないと、待ち始めた時点の先着を
掴んだまま、その変換が settle するまで保存が返らない。

抑止（変換中は選び直しも削除も受け付けない）はライブラリの仕様にしない。それはアプリの UI 方針で、
ライブラリが保証するのは競合が起きないことと、受け付けたときの結果だけ。
