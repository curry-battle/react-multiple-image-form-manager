...
React + Vite 環境で TanStack Form と Valibot を使い、複数画像アップロードを取り扱うフォーム実装のサンプル。

複数画像を取り扱うユーザのプロフィールを更新するというユースケースを想定した実装をしている。

React Hook Form 版は [`examples/profile-form`](../profile-form/README.ja.md) にある。扱う問題と画面は同じで、フォームライブラリだけが違う。

## 概要

配列フィールドで画像を扱うと実装がぐちゃぐちゃになりやすい。
...
## TanStack Form 版で違うところ

**`useTanstackMultiImageController` はフォームレベルで呼ぶ。`<form.Field mode="array">` の中に置く必要はない。**

```tsx
const form = useForm({ defaultValues, validators: { onChange: userProfileSchema } });

// form.Field の外。コンポーネント直下で呼べる
...
adapter は読み書きをすべて `form` 経由で行い、購読も `form.store` の `values` / `fieldMeta` から取る。配列フィールドの `field` オブジェクトを必要としないため、配列用のラッパーが要らない。検証も `<form.Field mode="array">` が無い状態で `validateField` がフォームレベル検証にフォールバックする（TanStack 側の `FormApi.validateField`）。

その他の差分:

| | TanStack Form 版 | React Hook Form 版 |
|---|---|---|
| 検証スキーマ | Valibot | Zod |
| 単一フィールドの描画 | `<form.Field name="name">` の render-props | `register("name")` |
| 送信ボタンの状態 | `<form.Subscribe selector={...}>` | `formState` |
| 配列の書き戻し | `form.setFieldValue("profileImages", next)` | `setValue` / `replace` |
| 起動コマンド | `pnpm run dev:example:tanstack` | `pnpm run dev:example:rhf` |

`items` / `handlers` / `uploads` / `raw` の形は両者で同じ。**ライブラリ本体の API はフォームライブラリに依存しない。**
...
[保存ボタン]
  ...
  promoteSavedImages(savedIds)                    ProfileForm.tsx
    → 登録が済んだ new 項目を existing へ昇格させる
```

`promoteSavedImages` を省くと、続けてもう一度保存したときに同じ画像が新規として再送される。
...
src/
	...
	components/
		...
		ProfileForm.tsx         # TanStack Formを使うFormのルート
	types/
		UserProfile.ts          # フォームデータの型
		schemas/
			UserProfileSchema.ts    # フォームのValibotスキーマ
	libs/
...
Playwrightを使ったE2Eテストを用意している。RHF 版と TanStack 版は project で分かれている。
